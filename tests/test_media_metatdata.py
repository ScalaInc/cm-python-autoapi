...
namespace = config['test']['namespace'] + datetime.datetime.now().strftime("%Y_%m_%d_%H%S.%f")
...
class TestMediaMetadataEndpoint():
    ...
    def setup(self):
        ...
        path = namespace + "endpoint_media_metadata" + cur_datetime.strftime("%Y%m%d_T%H%M%S")
        ...
        self.media_metadata_id_list = [self.int_any_id, self.int_picklist_id, self.boolean_metadata_id,
    ...
    def teardown(self):
        ...
        for metadata_id in self.media_metadata_id_list:
            logging.debug('attempting to DELETE media_metadata with id = {}'.format(metadata_id))
            ...
            assert media_metadata_object.create_metadata(session=self.test_session,
                                                         ...
                                                         value_type=metadata[
                                                             1]), 'Could not add media metadata object type ' + \
                                                                  metadata[0].name + ' value ' + metadata[1].name

            self.media_metadata_id_list.append(media_metadata_object.get_id())
    ...
    def test_delete_metadata(self):
        ...
        for metadata in test_data:
            ...
            self.media_metadata_id_list.append(media_metadata_object.get_id())
            self.delete_ids.append(media_metadata_object.get_id())
            logging.debug('adding {} to delete IDs'.format(media_metadata_object.get_id()))
        ...
        for metadata_id in self.delete_ids:
            assert media_metadata_object.delete_metadata_by_id(session=self.test_session,
                                                               baseurl=self.baseurl,
                                                               metadata_id=metadata_id), 'Could not delete metadata id with id = ' + str(metadata_id)

    def test_add_picklist_values_to_media_integer_metadata(self):
        '''
        ...
        assert media_metadata_object.add_picklist_values_to_media_metadata(session=self.test_session,
                                                                           baseurl=self.baseurl,
                                                                           media_metadata_id=self.int_picklist_id,
                                                                           list_of_predefined_values=test_data_list), 'Call to add picklist items to integer picklist media metadata did not return 200'

        media_metadata_object.find_metadata_by_id(session=self.test_session,
                                                  baseurl=self.baseurl,
                                                  metadata_id=self.int_picklist_id)

        list_of_metadata_picklist_values = [predefined_value['value'] for predefined_value in
                                            media_metadata_object.last_response.json()['predefinedValues']]
        ...
        for test_data_item in test_data_list:
            ...
            assert str(
                test_data_item) in list_of_metadata_picklist_values, 'Did not find test data in integer picklist metadata.  Searching for ' + str(
                test_data_item)

    def test_add_picklist_values_to_media_string_metadata(self):
        '''
        Endpoint test for /api/rest/metadata/multiple/{metadataId}
        :return:
        '''
        ...
        assert media_metadata_object.add_picklist_values_to_media_metadata(session=self.test_session,
                                                                           baseurl=self.baseurl,
                                                                           media_metadata_id=self.string_picklist_id,
                                                                           list_of_predefined_values=test_data_list), 'Call to add picklist items to string picklist media metadata did not return 200'

        media_metadata_object.find_metadata_by_id(session=self.test_session,
                                                  baseurl=self.baseurl,
                                                  metadata_id=self.string_picklist_id)

        list_of_metadata_picklist_values = [predefined_value['value'] for predefined_value in media_metadata_object.last_response.json()['predefinedValues']]
        logging.debug("List of picklist items pulled back from CM is: {}".format(list_of_metadata_picklist_values))
    ...
    def test_find_metadata_by_id(self):
        ...
        for media_metadata_id in self.media_metadata_id_list:
            assert media_metadata_object.find_metadata_by_id(session=self.test_session,
                                                             baseurl=self.baseurl,
    ...
    def test_list_picklist_values_by_metadata_id_integer(self):
        ...
        media_metdata_object = Media_meta_data(api_version_media_metadata)

        assert media_metdata_object.list_picklist_values_by_metadata_id(session=self.test_session,
                                                                        baseurl=self.baseurl,
        ...
        list_of_returned_picklist_values = [metadata_value['value'] for metadata_value in
                                            media_metdata_object.last_response.json()['list']]

        for metadata_value in self.int_picklist_items:
            assert str(metadata_value) in list_of_returned_picklist_values, 'Did not find ' + str(
                metadata_value) + ' in list picklist items response'
    ...
    def test_list_picklist_values_by_metadata_id_string(self):
        ...
        for metadata_value in self.string_picklist_items:
            logging.debug('Testing to see if {} is in the list of responses from CM: {}'.format(metadata_value,
                                                                                                list_of_returned_picklist_values))
            assert metadata_value in list_of_returned_picklist_values, 'Did not find ' + metadata_value + ' in list picklist items response'

    def test_list_media_metadata(self):
        '''
        ...
        returned_metadata_id_list = [metadata['id'] for metadata in media_metadata_object.get_response_key('list')]

        for media_metadata_id in self.media_metadata_id_list:
            logging.debug('Testing for {} in {}'.format(media_metadata_id, returned_metadata_id_list))
            assert media_metadata_id in returned_metadata_id_list, 'Could not find media id ' + str(
                media_metadata_id) + ' in ' + str(returned_metadata_id_list)
    ...
    def test_update_media_metadata(self):
        ...
        assert media_metadata_object.update_media_metadata(session=self.test_session,
                                                           baseurl=self.baseurl,
                                                           media_metadata_id=self.string_any_metadata_id,
        ...
        name_after_update = media_metadata_object.get_response_key('name')

        logging.debug('After update, expected: {}, got: {}'.format("MediaItem." + new_name, name_after_update))

        assert name_after_update == "MediaItem." + new_name, 'After update, name did not change correctly.  Expected ' + new_name + ' got ' + name_after_update
    ...
    def test_update_sort_order(self):
        ...
        media_metadata_object.list_media_metadata(session=self.test_session,
                                                  baseurl=self.baseurl,
                                                  limit=500,
                                                  fields='id,order',
                                                  sort='order')

        list_of_metadata = media_metadata_object.get_response_key('list')
        ...
        sort_metadata_order_param = [{'id': first_metadata_id, 'order': last_metadata_order},
                                     {'id': last_metadata_id, 'order': first_metadata_order}]

        logging.debug(
            'About to send the swap metadata order with the following parameter: {}'.format(sort_metadata_order_param))

        assert media_metadata_object.update_sort_order_media_metadata(session=self.test_session,
                                                                      baseurl=self.baseurl,
        ...
        new_list_of_metadata = media_metadata_object.get_response_key('list')

        assert new_list_of_metadata[0][
                   'order'] == last_metadata_order, 'After swapping the order of first and last media metadata, could not confirm order change in DTO'
        assert new_list_of_metadata[-1][
                   'order'] == first_metadata_order, 'After swapping the order of first and last media metadata, could not confirm order change in DTO'
